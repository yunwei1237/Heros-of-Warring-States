...
def createTroops(troopsKey,maxNumKey=None):
    ...
    for fac_index in range(0,len(factions)):
        ...
        for trp_index in range(len(lords),max_num):
            list.append(createTroop(lords, trp_index, troopsKey, fac, fac_index, fac_id))
...
def updateMod():
    for updateItem in updateItems:
        file_util.writeInRange(
            os.path.join(config.modpath,updateItem[0]),
            concatData(updateItem[1]),
            updateItem[2],
            updateItem[3])
    ##写入常量信息
    file_util.writeReplace(config.modpath +r"\module_constants.py",dataList["constants"])